...
const {addCompanyService, getCompaniesService, createJobService, updateJobService} = require('./job.services')
...
const updateJob = (req : Request, res : Response) => {
    const body = req.body
    updateJobService(body, (err : Error, results : {}) => {
        if(err){
            res.status(500).json({
                success : false,
                message : "Database Error!"
...
module.exports = {addCompany, getJobs, getCompanies, createJob, updateJob}